...
def _row_to_trade(r) -> dict:
    return {
        ...
        "accountId": r["account_id"], "brokerId": r["broker_id"], "settled": r["settled"],
    }
...
def create_trade(code: str, trade: TradeIn):
    with get_db() as conn:
        conn.execute(
            "INSERT INTO trades(id, code, date, type, shares, price, fee, sig_ref, note, account_id, broker_id, settled)"
            ...
            "  broker_id=EXCLUDED.broker_id, settled=EXCLUDED.settled",
            (trade.id, code, trade.date, trade.type, trade.shares,
             trade.price, trade.fee, trade.sigRef, trade.note, trade.accountId, trade.brokerId, trade.settled),
        )

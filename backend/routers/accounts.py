...
from models import AccountIn, AccountPatch, AccountOut, ReorderIn
...
def get_accounts():
    with get_db() as conn:
        rows = conn.execute("SELECT * FROM accounts ORDER BY sort_order ASC, name ASC").fetchall()
...
def create_account(account: AccountIn):
    with get_db() as conn:
        max_order = conn.execute("SELECT COALESCE(MAX(sort_order), -1) FROM accounts").fetchone()[0]
        conn.execute(
            "INSERT INTO accounts(id, name, balance, interest_rate, note, sort_order)"
            " VALUES (%s,%s,%s,%s,%s,%s)"
            " ON CONFLICT(id) DO UPDATE SET"
            "  name=EXCLUDED.name, balance=EXCLUDED.balance,"
            "  interest_rate=EXCLUDED.interest_rate, note=EXCLUDED.note",
            (account.id, account.name, account.balance, account.interestRate, account.note, max_order + 1),
        )
...
def reorder_accounts(body: ReorderIn):
    with get_db() as conn:
        for i, aid in enumerate(body.ids):
            conn.execute("UPDATE accounts SET sort_order=%s WHERE id=%s", (i, aid))
    return {"ok": True}
